...
export default function StatsScreen() {
  ...
  const { transactions, getMonthlyExpenses, getMonthlyIncome } =
    useFinanceStore();
  ...
  const last7 = Array.from({ length: 7 }, (_, i) => {
    ...
    const done = tasks.filter(
      (t) => t.dueDate.startsWith(ds) && t.completed,
    ).length;
    return {
      value: done,
      label: ["ح", "ن", "ث", "ر", "خ", "ج", "س"][d.getDay()],
      frontColor: Colors.primary,
    };
  });
          ...
          <AnimatedCard delay={0}>
            <View style={{ flexDirection: "row", gap: 10 }}>
              {[
                ...
                {
                  label: "وفرت",
                  value: saved > 0 ? formatAmountAr(saved) : "٠ج",
                  emoji: "💰",
                },
                {
                  label: "أطول streak",
                  value:
                    habits.length > 0
                      ? `${Math.max(...habits.map((h) => getStreak(h.id)))} يوم`
                      : "٠",
                  emoji: "🔥",
                },
              ].map((item) => (
