...
export function WalletCard({ wallet }: { wallet: Wallet }) {
  ...
  return (
    ...
    >
      <Text style={{ fontSize: 36, marginLeft: 14 }}>{WALLET_ICONS[wallet.type]}</Text>
      <View style={{ flex: 1 }}>
        <Text style={{ fontFamily: "Cairo-Bold", fontSize: 16, color: Colors.textPrimary }}>
          {wallet.name}
        </Text>
        <Text
          style={{ fontFamily: "Cairo-Regular", fontSize: 12, color: Colors.textSecondary }}
        >
      ...
      <View style={{ alignItems: "flex-end", gap: 2 }}>
        ...
        <Text style={{ fontFamily: "Cairo-Regular", fontSize: 11, color: Colors.textMuted }}>
          {balanceHidden ? "••••" : `المتاح ${formatAmountAr(availableBalance)}`}
        </Text>
        {wallet.reservedBalance > 0 && (
          <Text style={{ fontFamily: "Cairo-Regular", fontSize: 11, color: Colors.primary }}>
            {balanceHidden ? "••••" : `المحجوز ${formatAmountAr(wallet.reservedBalance)}`}
          </Text>
        )}
      </View>
    </View>
