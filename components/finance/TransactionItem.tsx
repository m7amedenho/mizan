...
const modePrefix = (t: Transaction) =>
  t.walletImpactMode === "record_only" ? "تسجيل فقط · " : "";
...
const getTransactionMeta = (transaction: Transaction) => {
  ...
  const autoSaveNote = transaction.autoSavedAmount
    ? ` · تحويش ${formatAmountAr(transaction.autoSavedAmount)}`
    : "";

  return {
    emoji: category?.emoji ?? "📦",
    subtitle: `${modePrefix(transaction)}${category?.label ?? transaction.category}${autoSaveNote}`,
    color: transaction.type === "income" ? Colors.primaryMid : Colors.danger,
    amountPrefix: transaction.type === "income" ? "+" : "-",
  };
};
